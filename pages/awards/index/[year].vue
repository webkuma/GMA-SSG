...
import YearAwardsData from "@/components/AwardsView/YearAwardsData.vue";
import emitter from "@/components/utils/emitter.js";
// 監聽子組件 AwardsDetailView；用以表示查無此年份
emitter.on("isNotFoundYear", (e) => {
  isNotFoundYear.value = e;
});
// 監聽子組件 YearSelect；onMounted 時預設參數是路由年份，onChange 時參數是下拉選單的值
emitter.on("yearChange", (e) => {
  selectedYear.value = e;
});
const router = useRouter();
const selectedYear = ref(); // 目前的年份
const isNotFoundYear = ref(false);
...
<template>
  <main class="text-white">
    ...
    <div
      v-if="isNotFoundYear"
      class="z-20 absolute w-full h-full top-0 bg-[#202020]/90 bg-opacity-90">
      <div class="flex flex-col items-center justify-center h-full text-white">
        <p class="text-6xl text-custom-gold">(='X'=)</p>
        <p class="text-2xl m-4">年份錯誤（無此年份）</p>
        <p class="text-lg">
          <RouterLink
            to="/"
